...
def quantify_emission(iso3):
    """
    This function quantifies 
    the total cost of ownership 
    for a single fiber broadband user
    """
    print('Generating emissions results for {}'.format(iso3))
    line_folder = os.path.join(DATA_RESULTS, iso3, 'gid_lines')
    gdf = gpd.GeoDataFrame()
    for file_name in os.listdir(line_folder):
    ...
    for file_name in os.listdir(pop_folder):
        ...
        if file_name.endswith('_demand_user.csv'):

            file_path = os.path.join(pop_folder, file_name) 
            df1 = pd.read_csv(file_path)
            df = df1.merge(gdf, on = 'GID_1', how = 'outer').reset_index(drop = True)
            df.fillna(df.mean(), inplace = True)
           
            df[['fiber_nodes', 'optic_fiber_mfg_ghg', 
                'aluminium_mfg_ghg', 'steel_iron_mfg_ghg', 
                'plastics_mfg_ghg', 'other_metals_mfg_ghg', 
                'concrete_mfg_ghg', 'total_mfg_ghg_kg',
                'optic_fiber_eolt_ghg', 'aluminium_eolt_ghg', 
                'steel_iron_eolt_ghg', 'plastics_eolt_ghg', 
                'other_metals_eolt_ghg', 'concrete_eolt_ghg', 
                'total_eolt_ghg_kg', 'total_trans_ghg_kg', 
                'total_ops_ghg_kg', 'total_ghg_emissions_kg', 
                'emissions_kg_per_subscriber']] = ''
            
            for i in range(len(df)):

                # 130.239564 is the coverage area of a 4G base 
                # station assuming the coverage radius is 4miles
                ################################ LCA MANUFACTURING PHASE ####################################
                df['fiber_nodes'].loc[i] = round((df['area'].loc[i]) / (130.239564))
                df['optic_fiber_mfg_ghg'].loc[i] = (lca_mfg['optic_fiber_ghg'] * (df['length'].loc[i] / 1000))
                df['aluminium_mfg_ghg'].loc[i] = (lca_mfg['aluminium_ghg'] * df['fiber_nodes'].loc[i])
                df['steel_iron_mfg_ghg'].loc[i] = (lca_mfg['steel_iron_ghg'] * df['fiber_nodes'].loc[i])
                df['plastics_mfg_ghg'].loc[i] = (lca_mfg['plastics_ghg'] * df['fiber_nodes'].loc[i])
                df['other_metals_mfg_ghg'].loc[i] = (lca_mfg['other_metals_ghg'] * df['fiber_nodes'].loc[i])
                df['concrete_mfg_ghg'].loc[i] = (lca_mfg['concrete_ghg'] * df['fiber_nodes'].loc[i])
                df['total_mfg_ghg_kg'].loc[i] = (df['optic_fiber_mfg_ghg'].loc[i] + df['aluminium_mfg_ghg'].loc[i] 
                                              + df['steel_iron_mfg_ghg'].loc[i] + df['plastics_mfg_ghg'].loc[i] 
                                              + df['other_metals_mfg_ghg'].loc[i] + df['concrete_mfg_ghg'].loc[i])

                ################################### LCA TRANSPORTATION PHASE #################################
                df['total_trans_ghg_kg'].loc[i] = (lca_tran['optic_fiber_ghg'] * df['fiber_nodes'].loc[i])

                ################################### LCA OPERATIONS PHASE #################################
                df['total_ops_ghg_kg'].loc[i] = (((lca_ops['cpe_power'] * df['fiber_nodes'].loc[i]) 
                                                  + ((lca_ops['base_station_power_kwh'] * df['fiber_nodes'].loc[i]) 
                                                  / (df['users_area_sqkm'].loc[i])) + (1.5 
                                                  * ((lca_ops['terminal_unit_pwr_kwh'] * df['fiber_nodes'].loc[i]) 
                                                  / df['users_area_sqkm'].loc[i]))) * 0.19338) 

                ############################ LCA END OF LIFE TREATMENT PHASE #################################
                df['optic_fiber_eolt_ghg'].loc[i] = (lca_eot['optic_fiber_ghg'] * (df['length'].loc[i] / 1000))
                df['aluminium_eolt_ghg'].loc[i] = (lca_eot['aluminium_ghg'] * df['fiber_nodes'].loc[i])
                df['steel_iron_eolt_ghg'].loc[i] = (lca_eot['steel_iron_ghg'] * df['fiber_nodes'].loc[i])
                df['plastics_eolt_ghg'].loc[i] = (lca_eot['plastics_ghg'] * df['fiber_nodes'].loc[i])
                df['other_metals_eolt_ghg'].loc[i] = (lca_eot['other_metals_ghg'] * df['fiber_nodes'].loc[i])
                df['concrete_eolt_ghg'].loc[i] = (lca_eot['concrete_ghg'] * df['fiber_nodes'].loc[i])
                df['total_eolt_ghg_kg'].loc[i] = (df['optic_fiber_eolt_ghg'].loc[i] + df['aluminium_eolt_ghg'].loc[i] 
                                                  + df['steel_iron_eolt_ghg'].loc[i] + df['plastics_eolt_ghg'].loc[i] 
                                                  + df['other_metals_eolt_ghg'].loc[i] + df['concrete_eolt_ghg'].loc[i])
                
                df['total_ghg_emissions_kg'].loc[i] = (df['total_mfg_ghg_kg'].loc[i] + df['total_trans_ghg_kg'].loc[i] 
                                                       + df['total_ops_ghg_kg'].loc[i] + df['total_eolt_ghg_kg'].loc[i])
                df['emissions_kg_per_subscriber'].loc[i] = (df['total_ghg_emissions_kg'].loc[i] / df['users_area_sqkm'].loc[i])
           
            df1 = df[['iso3', 'GID_1', 'pop_density', 'users_area_sqkm', 
                     'geotype', 'monthly_traffic', 'area', 'length',
                     'adoption_scenario', 'fiber_nodes', 'total_mfg_ghg_kg', 
                     'total_trans_ghg_kg', 'total_ops_ghg_kg', 
                     'total_eolt_ghg_kg', 'total_ghg_emissions_kg', 
                     'emissions_kg_per_subscriber']]
            
            df2 = df[['iso3', 'GID_1', 'pop_density', 'users_area_sqkm', 
                     'geotype', 'monthly_traffic', 'area', 'length',
                     'adoption_scenario', 'fiber_nodes', 'optic_fiber_mfg_ghg', 
                     'aluminium_mfg_ghg', 'steel_iron_mfg_ghg', 
                     'plastics_mfg_ghg', 'other_metals_mfg_ghg', 
                     'concrete_mfg_ghg', 'total_mfg_ghg_kg']]
            
            df3 = df[['iso3', 'GID_1', 'pop_density', 'users_area_sqkm', 
                     'geotype', 'monthly_traffic', 'area', 'length',
                     'adoption_scenario', 'fiber_nodes', 'optic_fiber_eolt_ghg', 
                     'aluminium_eolt_ghg', 'steel_iron_eolt_ghg', 
                     'plastics_eolt_ghg', 'other_metals_eolt_ghg', 
                     'concrete_eolt_ghg', 'total_eolt_ghg_kg']]
            
            df2 = pd.melt(df2, id_vars = ['iso3', 'GID_1', 'pop_density', 
                     'users_area_sqkm', 'geotype', 'monthly_traffic', 'area', 
                     'length', 'adoption_scenario', 'fiber_nodes', 'total_mfg_ghg_kg'], 
                     value_vars = ['optic_fiber_mfg_ghg', 'aluminium_mfg_ghg', 
            ...
            df3 = pd.melt(df3, id_vars = ['iso3', 'GID_1', 'pop_density', 
                     'users_area_sqkm', 'geotype', 'monthly_traffic', 'area', 
                     'length', 'adoption_scenario', 'fiber_nodes', 'total_eolt_ghg_kg'], 
                     value_vars = ['optic_fiber_eolt_ghg', 'aluminium_eolt_ghg', 
                     'steel_iron_eolt_ghg', 'plastics_eolt_ghg', 'other_metals_eolt_ghg', 
                     'concrete_eolt_ghg',], var_name = 'emission_category', 
                     value_name = 'lca_phase_ghg_kg')
            
            merged_dataframe1 = pd.concat([merged_dataframe, df1], ignore_index = True)
            merged_dataframe2 = pd.concat([merged_dataframe, df2], ignore_index = True)
            merged_dataframe3 = pd.concat([merged_dataframe, df3], ignore_index = True)

            fileout = '{}_emission_results.csv'.format(iso3)
            fileout1 = '{}_mfg_emission_results.csv'.format(iso3)
            fileout2 = '{}_eolt_emission_results.csv'.format(iso3)
            ...
            merged_dataframe1.to_csv(path_out, index = False)
            merged_dataframe2.to_csv(path_out1, index = False)
            merged_dataframe3.to_csv(path_out2, index = False)
