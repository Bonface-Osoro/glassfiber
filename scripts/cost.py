...
def demand_point(iso3):
    """
    This function generate a csv file 
    for plotting demand results.
    ...
    line_folder = os.path.join(DATA_RESULTS, iso3, 'country_points')
    ...
    merged_gdf = merged_gdf[['iso3', 'GID_1', 'population', 'area', 'geometry']]
    merged_gdf[['area']] = merged_gdf[['area']].round(4)

    fileout = '{}_mapping_points.csv'.format(iso3, merged_shapefile).replace('shp', '_')
...
def demand_mapping(iso3):
    ...
    for idx, country in countries.iterrows():
        ...
        for file_name in os.listdir(line_folder):
            ...
            if file_name.endswith('_mapping_points.csv'):
                ...
                df = pd.melt(df, id_vars = ['iso3', 'GID_1', 'population', 
                        'area', 'geometry'], var_name = 'adoption_scenario', 
                ...
                df = df[['iso3', 'GID_1', 'population', 'area', 'adoption_scenario', 
        ...
        fileout = '{}_mapping_results.csv'.format(iso3)
        folder_out = os.path.join(DATA_RESULTS, iso3, 'demand')
...
if __name__ == '__main__':
    ...
    for idx, country in countries.iterrows():
        ...
        demand_point(countries['iso3'].loc[idx])
        #demand_mapping(countries['iso3'].loc[idx])