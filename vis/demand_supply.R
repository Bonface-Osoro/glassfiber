...
new_names <- c('iso3', 'country', 'GID_1', 'GID_2', 'geometry')
colnames(africa_shp) <- new_names
data <- read.csv(file.path(folder, '..', 'results', 'SSA', 'SSA_pcsf_local_tco_results.csv'))
...
pcsf_maps <- ggplot() + 
  geom_sf(data = africa_shp, fill = "yellowgreen", color = "black", linewidth = 0.01) + 
  geom_sf(data = merged_data, aes(fill = population_bin), 
          linewidth = 0.001,) +
  scale_fill_brewer(palette = "Spectral") +
  labs(title = "(c) TCO per user: Djikistra algorithm.",
