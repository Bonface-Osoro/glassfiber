...
NGA_access_prims_fiber <- ggplot() +
  ...
  labs(title = "(A) Nigeria", 
       subtitle = "Minimum Spanning Tree (MST) Prim's algorithm", color = "Network Level") + 
...
NGA_access_pcsf_fiber <- ggplot() +
  ...
  geom_sf(data = core_edges, aes(color = Type), linewidth = 0.3, show.legend = TRUE) + 
  labs(title = " ", 
       subtitle = "Prize Collecting Steiner Tree (PCST) algorithm", color = "Network Level") + 
  scale_color_manual(values = c("Existing Fiber Line" = "green4", 
                                "Designed Fiber Access Lines" = "darkorange")) +
  theme(
    axis.title.y = element_text(size = 6),
...
########################################
##PRIM'S FIBER INFRASTRUCTURE DRC MAP ##
########################################
DRC_data <- st_read(file.path(folder, '..', 'results', 'processed', 'COD', 
                              'regions', 'regions_2_COD.shp'))

access_nodes <- st_read(file.path(folder, '..', 'results', 'processed', 'COD',
                                  'buffer_routing_zones', 'combined',
                                  'COD_combined_access_nodes.shp'))
access_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'COD', 
                                  'buffer_routing_zones', 'combined',
                                  'COD_combined_access_edges.shp'))
core_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'COD',
                                'network_existing',
                                'COD_core_edges_existing.shp'))
core_edges$Type <- 'Existing Fiber Line'
access_edges$Type <- 'Designed Fiber Access Lines'

DRC_access_prims_fiber <- ggplot() +
  geom_sf(data = DRC_data, fill = "gray96", color = "black", linewidth = 0.05) +
  geom_sf(data = access_nodes, size = 0.05) + 
  geom_sf(data = access_edges, aes(color = Type), size = 0.3, linewidth = 0.5, show.legend = TRUE) + 
  geom_sf(data = core_edges, aes(color = Type), linewidth = 0.3, show.legend = TRUE) +
  labs(title = "(C) Democratic Republic of Congo (DRC)", 
       subtitle = "Minimum Spanning Tree (MST) Prim's algorithm", color = "Network Level") + 
  scale_color_manual(values = c("Existing Fiber Line" = "green4", 
                                "Designed Fiber Access Lines" = "darkorange")) +
  theme(
...
access_nodes <- st_read(file.path(folder, '..', 'results', 'processed', 'COD',
                                  'buffer_routing_zones', 'combined',
                                  'COD_combined_pcsf_subregional_nodes.shp'))
access_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'COD', 
                                  'buffer_routing_zones', 'combined',
                                  'COD_combined_pcsf_access_edges.shp'))
core_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'COD',
                                'network_existing',
                                'COD_core_edges_existing.shp'))
...
#############################################
##PRIM'S FIBER INFRASTRUCTURE TANZANIA MAP ##
#############################################
TZA_data <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA', 
                              'regions', 'regions_2_TZA.shp'))

access_nodes <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA',
                                  'buffer_routing_zones', 'combined',
                                  'TZA_combined_access_nodes.shp'))
access_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA', 
                                  'buffer_routing_zones', 'combined',
                                  'TZA_combined_access_edges.shp'))
core_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA',
                                'network_existing',
                                'TZA_core_edges_existing.shp'))
core_edges$Type <- 'Existing Fiber Line'
access_edges$Type <- 'Designed Fiber Access Lines'

TZA_access_prims_fiber <- ggplot() +
  geom_sf(data = TZA_data, fill = "gray96", color = "black", linewidth = 0.05) +
  geom_sf(data = access_nodes, size = 0.05) + 
  geom_sf(data = access_edges, aes(color = Type), size = 0.3, linewidth = 0.5, show.legend = TRUE) + 
  geom_sf(data = core_edges, aes(color = Type), linewidth = 0.3, show.legend = TRUE) +
  labs(title = "(D) Tanzania", 
       subtitle = "Minimum Spanning Tree (MST) Prim's algorithm", color = "Network Level") + 
  scale_color_manual(values = c("Existing Fiber Line" = "green4", 
                                "Designed Fiber Access Lines" = "darkorange")) +
  theme(
...
access_nodes <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA',
                                  'buffer_routing_zones', 'combined',
                                  'TZA_combined_pcsf_subregional_nodes.shp'))
access_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA', 
                                  'buffer_routing_zones', 'combined',
                                  'TZA_combined_pcsf_access_edges.shp'))
core_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'TZA',
                                'network_existing',
                                'TZA_core_edges_existing.shp'))
...
#################################################
##PRIM'S FIBER INFRASTRUCTURE SOUTH AFRICA MAP ##
#################################################
ZAF_data <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF', 
                              'regions', 'regions_2_ZAF.shp'))

access_nodes <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF',
                                  'buffer_routing_zones', 'combined',
                                  'ZAF_combined_access_nodes.shp'))
access_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF', 
                                  'buffer_routing_zones', 'combined',
                                  'ZAF_combined_access_edges.shp'))
core_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF',
                                'network_existing',
                                'ZAF_core_edges_existing.shp'))
core_edges$Type <- 'Existing Fiber Line'
access_edges$Type <- 'Designed Fiber Access Lines'

ZAF_access_prims_fiber <- ggplot() +
  geom_sf(data = ZAF_data, fill = "gray96", color = "black", linewidth = 0.05) +
  geom_sf(data = access_nodes, size = 0.05) + 
  geom_sf(data = access_edges, aes(color = Type), size = 0.3, linewidth = 0.5, show.legend = TRUE) + 
  geom_sf(data = core_edges, aes(color = Type), linewidth = 0.3, show.legend = TRUE) +
  labs(title = "(E) South Africa", 
       subtitle = "Minimum Spanning Tree (MST) Prim's algorithm", color = "Network Level") + 
  scale_color_manual(values = c("Existing Fiber Line" = "green4", 
                                "Designed Fiber Access Lines" = "darkorange")) +
  theme(
...
access_nodes <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF',
                                  'buffer_routing_zones', 'combined',
                                  'ZAF_combined_pcsf_subregional_nodes.shp'))
access_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF', 
                                  'buffer_routing_zones', 'combined',
                                  'ZAF_combined_pcsf_access_edges.shp'))
core_edges <- st_read(file.path(folder, '..', 'results', 'processed', 'ZAF',
                                'network_existing',
                                'ZAF_core_edges_existing.shp'))
